 ...
 * @notice Interface of the protocol vault
...
interface IVault is IERC4626 {
    ...
    /**
     * @notice Vault configuration parameters specific to the risk tier, passed in the constructor
     * @param idle_threshold Idle buffer threshold to execute allocateIdle
     * @param min_profit_for_harvest Minimum profit required to execute harvest
     * @param max_tvl Maximum allowed TVL as a circuit breaker
     * @param min_deposit Minimum allowed deposit
     */
    struct TierConfig {
     ...
     * @param timestamp Timestamp of the moment the harvest was executed
     */
    ...
    /**
     * @notice Emitted when idle assets are allocated to strategies via the strategy manager
     * @param amount Amount of idle assets that were allocated to strategies
     */
    event IdleAllocated(uint256 amount);

    /**
     * @notice Emitted when idle_buffer is reconciled with the vault's real balance (after emergency exits)
     * @param old_buffer Value of idle_buffer before synchronization
     * @param new_buffer Value of idle_buffer after synchronization (real WETH balance)
     */
     ...
     * @dev Public function with no restrictions: any address can execute harvest for the
     *      benefit of the protocol (incentivizing frequent execution via keeper bots)
     * @return profit Total profit harvested in assets before deducting performance fees
     */
    function harvest() external returns (uint256 profit);

    /**
     * @notice Allocates vault idle buffer assets to strategies via the strategy manager
     * @dev Only executes if idleBuffer >= idleThreshold, avoiding unnecessary gas on small allocations.
     *      Idle assets accumulate mainly from new user deposits that have not yet been
     *      allocated to productive strategies
    ...
    /**
     * @notice Updates the idle assets threshold for executing allocation
     * @dev Can only be called by the vault owner
    ...
    /**
     * @notice Reconciles idle_buffer with the contract's real WETH balance
     * @dev Can only be called by the owner. Executed after the manager's emergencyExit()
     */
     ...
     *      by strategies in each harvest and distributed between treasury and founder
     * @return fee_bps Performance fee in basis points
     */
    function performance_fee() external view returns (uint256 fee_bps);

    /**
     * @notice Percentage of the performance fee assigned to the treasury
     * @dev In basis points over the total performance fee (not over total profit).
     * @return split_bps Treasury split in basis points (must sum to BASIS_POINTS with founder_split)
     */
    function treasury_split() external view returns (uint256 split_bps);

    /**
     * @notice Returns the percentage of the performance fee assigned to the founder
     * @dev In basis points over the total performance fee (not over total profit).
     * @return split_bps Founder split in basis points (must sum to BASIS_POINTS with treasury_split)
     */
    function founder_split() external view returns (uint256 split_bps);

    /**
     * @notice Returns the minimum deposit allowed in the vault
     * @dev Prevents extremely small deposits that are not economically efficient
     *      due to gas cost. Deposits below this threshold will be reverted
     ...
     * @dev Prevents uneconomical allocations. Only when idleBuffer >= idleThreshold is the
     *      gas cost of allocating capital to strategies justified. Below this
     *      threshold, allocateIdle() will not execute anything
     * @return threshold Minimum idle asset limit to perform allocation
     */
     ...
     * @dev Safety limit to prevent excessive risk in early stage. Deposits that
     *      exceed this limit will be reverted, protecting the protocol while its
     *      robustness is proven. Can be progressively increased as the protocol matures
     ...
     *      These funds are typically used for protocol development, security and growth
    ...
    /**
     * @notice Returns the protocol founder or team address
     * @dev The founder receives their percentage (founder_split) of the generated performance fees.
     *      It is the reward for developing and maintaining the protocol
     ...
     * @dev The idle buffer represents liquidity available in the vault that is not generating yield.
     *      It accumulates from user deposits and is reduced via allocateIdle() when it reaches
     *      the idleThreshold. Maintaining a buffer allows fast withdrawals without withdrawing from strategies
     *      and saves a lot of gas by not making immediate user deposits into the protocols
     * @return idle_balance Current idle buffer asset balance in the vault
     */
     ...
     *      the next harvest based on reward accumulation in the strategies
    ...
    /**
     * @notice Returns the total profit accumulated since the vault's inception
     ...
     * @return total_profit Total accumulated profit in assets since vault deployment
     */
    function totalHarvested() external view returns (uint256 total_profit);

    /**
     * @notice Returns whether an address is an official protocol keeper
     * @dev Official keepers do not receive an incentive when executing harvest
     * @param keeper Address to check
     * @return is_official True if it is an official keeper, false otherwise
     */
     ...
     * @dev In basis points over total profit. External keepers that execute harvest
     *      receive this percentage as a reward. Official keepers receive no incentive
     * @return incentive_bps Keeper incentive in basis points
     */
